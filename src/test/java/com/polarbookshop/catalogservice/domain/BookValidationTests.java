...
class BookValidationTests {
    ...
    @BeforeAll//Идентифицирует блок кода, выполняемый перед всеми тестами в классе.
    static void setUp() {
    ...
    void whenAllFieldsCorrectThenValidationSucceeds() {
        var book = new Book("1234567890", "Title", "Author", 9.90);//Создает книгу с действительным ISBN.
        Set<ConstraintViolation<Book>> violations = validator.validate(book);
        assertThat(violations).isEmpty();//Утверждает, что нет ошибки проверки
    }

    @Test
    void whenIsbnNotDefinedThenValidationFails() {
        var book = new Book("", "Title", "Author", 9.90);// Создает книгу с недействительным кодом ISBN
        Set<ConstraintViolation<Book>> violations = validator.validate(book);
    ...
    void whenIsbnDefinedButIncorrectThenValidationFails() {
        ...
        assertThat(violations.iterator().next().getMessage())
                .isEqualTo("The ISBN format must be valid.");//Создает книгу с недействительным кодом ISBN
    }
